...
export default function Header() {
  ...
  // Lock scroll + horizontal wiggle while menu is open
  useEffect(() => {
    if (!open) return;
  ...
  // Same logo size on mobile in both closed/open header rows
  ...
  return (
    <header className="absolute inset-x-0 top-0 z-40">
      ...
      {/* FULL-SCREEN MOBILE MENU (covers edge-to-edge; no bottom gap) */}
      {open && (
        <div
          className="
          ...
          "
          // 100svh covers visual viewport on modern mobile; 100dvh as fallback
          style={{ height: "100svh", minHeight: "100dvh" }}
          aria-modal="true"
          role="dialog"
        >
          <div className="flex w-full flex-col" style={{ minHeight: "100svh" }}>
            {/* Top row identical to closed header (logo left, X right) */}
            ...
            {/* Bottom CTA with safe-area padding so the blue sheet covers bottom */}
